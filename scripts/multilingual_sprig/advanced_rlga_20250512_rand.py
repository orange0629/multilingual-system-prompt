...
gemma_template_task_only = '''<bos><start_of_turn>user\n{user_prompt}<end_of_turn>\n<start_of_turn>model\n'''
...
def random_mutate_batched(selected_batch, action, args):
    if "llama" in args.model_name.lower():
        ...
        local_template = gemma_template_task_only
...
No additional explanations or formatting, only return the list."""
        ...
        for output_idx in range(len(output_lst)):
            ...
            try:
                snippet = output[output.find('['):output.rfind(']') + 1]
                new = eval(snippet)
                assert isinstance(new, list), f"ERROR!!! new is not a list: {new} from {snippet}"
            except:
                new = []
            for comp in new:
                if isinstance(comp, str):
                    ...
                    print("ERROR!!!")
                    print(f"Component {comp} is not a string in {output}.\n\n\n")
...
as a Python list of strings. No additional explanations or formatting, only return the list."""
        ...
        for output_idx in range(len(output_lst)):
            ...
            try:
                snippet = output[output.find('['):output.rfind(']') + 1]
                new = eval(snippet)
                assert isinstance(new, list), f"new is not a list: {new} from {snippet}"
            except:
                new = []
            for comp in new:
                if isinstance(comp, str):
...
sentence. No additional explanations or formatting, only return a sentence."""
        ...
        for selected in selected_batch:
            if len(selected) == 0:
                print("ERROR!!!")
                print(f"Selected batch {selected} is empty.\n\n\n")
                continue
...
class GeneticRLPrompter:
    ...
    def run_model_eval_multigpu(self, system_prompts, task_queue, result_queue, benchmark_obj_list, model_name, split="all", random_seed_lst=None):
        ...
        metric_result_df["overall_score"] = (0.5 * normalized_metric_result_df["acc_mean"] + 0.25 * (1 - normalized_metric_result_df["acc_var"]) + 0.125 * (1 - normalized_metric_result_df["output_tokens_var"]) + 0.125 * normalized_metric_result_df["consistency"])
    ...
    def evolutionary_step(self, args):
        ...
        if args.retrain:
            ...
            # print the first 10 rows of the train_ds
            print(train_ds[:10])
            print(dev_ds[:10])

            # Evaluate the reward model on the dev set before training
            _, pred_scores = self.evaluate_with_reward_model([tmp["prompt"] for tmp in dev_rows])
            print("\nSpearman correlation per metric before training")
            weights = {
                    "acc_mean": 0.5,
                    "acc_var": -0.25,
                    "output_tokens_var": -0.125,
            ...
            overall_spearman_score = 0
            for i, m in enumerate(METRIC_COLS):
                ro, p = spearmanr(rankdata(pred_scores[m]), rankdata([tmp[m] for tmp in dev_rows]))
                print(f"{m:15s}: ro = {ro:.4f}  (p = {p:.3g})")
                wandb.log({f"rm_{m}_spearman_before_training": ro, "global_step": self.step_idx})
                overall_spearman_score += weights[m] * ro
            wandb.log({f"rm_overall_spearman_before_training": overall_spearman_score, "global_step": self.step_idx})
            ...
            for i, m in enumerate(METRIC_COLS):
                ro, p = spearmanr(rankdata(pred_scores[m]), rankdata([tmp[m] for tmp in dev_rows]))
                print(f"{m:15s}: ro = {ro:.4f}  (p = {p:.3g})")
                wandb.log({f"rm_{m}_spearman": ro, "global_step": self.step_idx})
                overall_spearman_score += weights[m] * ro
            wandb.log({f"rm_overall_spearman": overall_spearman_score, "global_step": self.step_idx})
            
            # save the trained model
            self.reward_model.save_pretrained(os.path.join(args.cache_dir, f"reward_model_{self.step_idx}_{overall_spearman_score:.4f}"))
