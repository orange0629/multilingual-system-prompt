...
def train_model(args):
    ...
    rows = [row for row in rows if row["model"] == args.col_name]
...
def test_model(args):
    ...
    if args.test_path is not None:
        ...
        rows = [row for row in rows if row["split"] == "train" and row["step"] == 1]
        # data = rows.to_dict(orient="records")
        train_rows, test_rows = train_test_split(rows, test_size=0.4, random_state=42)
    ...
    # print(f"Reward model evaluation complete. Score range: {min(preds)} to {max(preds)}")
...
def parse_args():
    ...
    p.add_argument("--col_name", type=str, help="JSONL column name for training", default="Llama-3.1-8B-Instruct")
